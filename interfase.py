...
def add_basket(sender, add_data, user_data):
    ...
    with dpg.group(horizontal=True, parent="Basket", tag=tag):
        dpg.add_button(
            label=data[user_data],
            height=height_Goods,
            width=width_Goods,
            callback=remowe_basket,
...
dpg.create_viewport(
    title="Buy with this product", width=N_column * (width_Goods + 10) + 50, height=1000
)
dpg.setup_dearpygui()
